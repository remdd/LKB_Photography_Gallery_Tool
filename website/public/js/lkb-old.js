...
function updateState(hideNav) {
	...
	} else if(lkb.state.view === 'photo') {
		$('#mainDiv').fadeOut('fast', () => {
			lkb.state.img.currentName = lkb.photos[lkb.img.current].name;
			$('.mainPane').hide();
			$('#fullImg').show();
...
function showPrevImg() {
	let num = lkb.state.img.current - 1 < 0 ? lkb.state.img.total -1 : lkb.state.img.current - 1;
	viewPhoto(num);
}

function showNextImg() {
	let num = lkb.state.img.current + 1 >= lkb.state.img.total ? 0 : lkb.state.img.current + 1;
	viewPhoto(num);
}
