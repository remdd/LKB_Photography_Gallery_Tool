...
Menu = function() {
	this.main = function() {
		...
		stdout.write("\nEnter '4' to copy all galleries to the website and exit.");
		stdout.write("\nEnter '0' to exit.\n");
		//	Get user choice
		let choice = this.getChoice(5, true);
		switch(choice) {
			case 0:
				this.exit();
				break;
	...
	this.copyGalleries = function() {
		stdout.write('\nCopying galleries...');
		// rimraf.sync(path.join(__dirname, '..', 'website', 'public', 'galleries'));
		let websiteGalleries = path.join(__dirname, '..', 'website', 'public', 'galleries');
		fs.removeSync(websiteGalleries);
		fs.copySync(lkb.path.full, websiteGalleries);
		this.exit();
	}
	this.exit = function() {
